function previous=get_previous_displacement_2D(Knots_n,griddim)

dim_r=griddim(1);
dim_c=griddim(2);

previous=zeros(dim_r*dim_c,2);
previous(:,1)=reshape(Knots_n(:,:,1),[dim_r*dim_c,1]);
previous(:,2)=reshape(Knots_n(:,:,2),[dim_r*dim_c,1]);
